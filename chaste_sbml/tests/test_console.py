...
def test_generate_emits_placeholder_test(tmp_path):
    """--tests produces a placeholder test file alongside the model."""
    result = subprocess.run(
        ["chaste-sbml", str(GOLDBETER), "--output-dir", str(tmp_path), "--tests"],
        capture_output=True,
        text=True,
    )
...
def test_generate_skips_placeholder_test_by_default(tmp_path):
    """Without --tests, generation writes the model but no placeholder test."""
    result = subprocess.run(
...
def test_generate_test_output_dir_routes_test(tmp_path):
    """--test-output-dir asks for a placeholder test and places it in its own directory."""
