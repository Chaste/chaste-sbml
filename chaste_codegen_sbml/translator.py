...
def GetConstDoubleDefinition(num_tabs, name, val, newline):
    ...
    const_double_def = (
        AddTabs(num_tabs) + "const double " + name + " = " + str(val) + ";"
    )
...
def GetStateParameterDefinition(num_tabs, n, sp, newline):
    """Get a string that defines the state variable in C++."""
    state_parameter_def = (
        AddTabs(num_tabs) + "double " + sp + " = this->mParameters[" + str(n) + "];"
    )
...
def GetReactionParameterString(reaction):
    ...
    for i in range(num_kinetic_law_parameters):
        ...
        parameter_id_and_value = GetDoubleDefinition(
            1, parameter_id, parameter_value, True
        )
...
def GetReactionString(model):
    ...
    reactions_string = GetBlockCommentDefinition(
        1, "Define the reactions in this model.", True
    )
...
def IsTimescaleDifferent(model):
    ...
    if num_unit_defns != 0:
        for i in range(num_unit_defns):
            ...
            if (
                unit_id.lower() == "time"
            ):  # Do people ever call this something different?
...
def AccountForTimescale(model):
    ...
    odes_dict = GetOdesDictionary(
        model
    )  # Get the dictionary describing the ode reactions
    num_species = len(
        odes_dict
    )  # This does not include the species defined in algebraic rules

    timescale_multiplier = GetTimescaleMultiplier(model)

    timescale_str = GetBlockCommentDefinition(
        1, "Account for the differences in timescales.", True
    )

    for i in range(num_species):
        timescale_str += (
            AddTabs(1) + "rDY[" + str(i) + "] *= " + timescale_multiplier + ";\n"
        )
...
def GetOdesString(model):
    ...
    for i in range(num_species):
        ...
        if (species_id in odes_dict) & (
            ...
            ############# EDITED ##############
            compartment_name = GetCompartmentNameCorrespondingToId(
                model, compartment_id
            )

            # Get the C++ definition for the ODE
            ode_def = GetOdeDefinition(
                1, ode_count, species_ode, compartment_name, False
            )
            ode_count += 1
        ...
        ########################## EDITED #############
            ...
            (IsVariableDefinedAsRule(species_id, model))
            & (not (species_id in odes_dict))
            & (not ((species_name == "drag") or (species_id == "drag")))
        ):

            rules_dict = GetRulesDictionary(model)

            rule_string = rules_dict[
                species_id
            ]  ## I assume these are assignments where variables are added together to represent a total: TODO: include other rules
            ...
            ode_def = GetOdeDefinitionFromAssignment(
                1, ode_count, species_ode, compartment_name, False
            )
            ode_count += 1
...
def GetStateVariableString(model):
    ...
    state_variables_string = GetBlockCommentDefinition(
        1, "Define state variables", True
    )
    ...
    for i in range(num_species):
        ...
        state_variable_def = GetStateVariableDefinition(
            1, state_variable_count, species_id, False
        )
        ...
        if state_variable_name:
            state_variables_string += " " + GetCommentDefinition(
                0, state_variable_name, True
            )
        else:
...
def IsSpeciesDefinedAsParameter(species_id, model):
    ...
    if (not IsSpeciesDefinedAsOde(species_id, model)) & (
        not IsVariableDefinedAsRule(species_id, model)
    ):
        return True
    else:
        return False
...
def GetStateParameterString(model):
    ...
    to C++ code."""
    state_parameters_string = GetBlockCommentDefinition(
        1, "Define state parameters", True
    )
    ...
    for i in range(num_species):
        ...
        if IsSpeciesDefinedAsParameter(species_id, model):
            state_parameter_def = GetStateParameterDefinition(
                1, state_parameter_count, species_id, False
            )
            ...
            if state_parameter_name:
                state_parameters_string += " " + GetCommentDefinition(
                    0, state_parameter_name, True
                )
            else:
    ...
    for i in range(num_parameters):
        ...
        if (not IsParameterValueSet(i, model)) & (
            ...
            state_parameter_def = GetStateParameterDefinition(
                1, state_parameter_count, parameter_id, False
            )
            state_parameter_count += 1
            state_parameters_string += state_parameter_def
            ...
            if (
                ...
                state_parameter_def = GetStateParameterDefinition(
                    1, state_parameter_count, parameter_name, False
                )
                state_parameter_count += 1
                state_parameters_string += state_parameter_def
                state_parameters_string += " " + GetCommentDefinition(
