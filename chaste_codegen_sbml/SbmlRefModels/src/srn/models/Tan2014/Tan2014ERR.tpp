...
void Tan2014OdeSystem::EvaluateYDerivatives(double time, const std::vector<double> &rY, std::vector<double> &rDY)
{
    ...
    double kN = nucleus * (kN_k1 * rY[3] * rY[4] - kN_k2 * rY[5]);

    double kdiffusion = kdiffusion_k * (rY[0] - rY[3]);

    double K_c_active = K_c_active_k * rY[0];

    double K_n_active = K_n_active_k * rY[3];
